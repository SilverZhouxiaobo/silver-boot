package cn.silver.framework.monitor.aspectj;

import cn.silver.framework.common.enums.HttpMethod;
...
public class LogAspect {
    ...
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, OperLog operLog, Object jsonResult) throws Exception {
        ...
        JSONObject result = JSON.parseObject(JSON.toJSONString(jsonResult));
        // 设置标题
        String title = log.title();
        if (StringUtils.isBlank(title) || "data".equalsIgnoreCase(title)) {
            title = MapUtils.getString(result, "title", "通用数据接口");
        }
