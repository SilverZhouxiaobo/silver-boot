package cn.silver.framework.common.utils;
...
public class QRCodeManageUtil {
    ...
    /**
     * @param model              二维码参数内容，如果是一个网页地址，如 https://www.baidu.com/ 则 微信扫一扫会直接进入此地址， 如果是一些参数，如
     *                           1541656080837，则微信扫一扫会直接回显这些参数值
     * @param codeImgFileSaveDir 二维码图片保存的目录,如 D:/codes
     * @param fileName           二维码图片文件名称，带格式,如 123.png
     */
    public static void createCodeToFile(QrCodeModel model, File codeImgFileSaveDir, String fileName) {
        try {
...
//                fileName = new Date().getTime() + ".png";
            ...
            BufferedImage bufferedImage = getBufferedImage(model);
     ...
     */
    public static void createCodeToOutputStream(QrCodeModel model, OutputStream outputStream) {
        try {
            BufferedImage bufferedImage = getBufferedImage(model);
            /*
    ...
    private static BufferedImage getBufferedImage(QrCodeModel model) throws WriterException {
        /*
        ...
        BitMatrix bitMatrix = multiFormatWriter.encode(model.getLink(), BarcodeFormat.QR_CODE, CODE_WIDTH, CODE_HEIGHT, hints);
        /*
...
//        outg.drawImage(image, 500 - CODE_WIDTH, 0, image.getWidth(), image.getHeight(), null);
//
//        // 设置文字颜色为黑色
...
//            outg.drawString("编号: " + model.getCode(), 20, 30);
//        }
//        if (StringUtils.isNotBlank(model.getName())) {
//            outg.drawString("名称: " + model.getName(), 20, 60);
//        }
//        if (StringUtils.isNotBlank(model.getContent())) {
//            outg.drawString("简介: " + model.getContent(), 20, 60);
//        }
...
//        image.flush();
    ...
    public static BufferedImage createQr(QrCodeModel model, Font font) throws WriterException {
        /**
         * MultiFormatWriter:多格式写入，这是一个工厂类，里面重载了两个 encode 方法，用于写入条形码或二维码
         *      encode(String contents,BarcodeFormat format,int width, int height,Map<EncodeHintType,?> hints)
         *      contents:条形码/二维码内容
         ...
         *      rgb：像素的值，采用 16 进制,如 0xFFFFFF 白色
         */
        BitMatrix bm = multiFormatWriter.encode(model.getContent(), BarcodeFormat.QR_CODE, CODE_WIDTH, CODE_HEIGHT, hints);
        // 创建一个图片缓冲区存放二维码图片
...
// ------------------------------------------自定义文本描述-------------------------------------------------
        if (StringUtils.isNotBlank(model.getCode())) {
            ...
            // 获取字体宽度
//            int codeWidth = outg.getFontMetrics().stringWidth(model.getCode());
//            int nameWidth = outg.getFontMetrics().stringWidth(model.getName());

            // drawString(文字信息、x轴、y轴)方法根据参数设置文字的坐标轴 ，根据需要来进行调整
            outg.drawString("编号: " + model.getCode(), 20, 30);
            outg.drawString("名称: " + model.getName(), 20, 60);
            outg.drawString("介绍: " + model.getContent(), 20, 60);
            //  例： outg.drawString(depatmentName,  65 - strWidth / 2, height + (outImage.getHeight() - height) / 2 - h3); 根据需求自行计算需要的宽高
