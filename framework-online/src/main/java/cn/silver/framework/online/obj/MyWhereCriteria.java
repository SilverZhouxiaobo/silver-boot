package cn.silver.framework.online.obj;

import cn.silver.framework.core.bean.Response;
import cn.silver.framework.online.exception.InvalidDataFieldException;
import cn.silver.framework.online.exception.InvalidDataModelException;
...
public class MyWhereCriteria {
     ...
     * @return 组装后的SQL条件从句。
     * @throws InvalidDataFieldException selectFieldList中存在非法实体字段时，抛出该异常。
     */
...
//    }
     ...
     * @param slaveColumnName
     * @param fieldName       条件所属的实体对象的字段名。
     * @param operatorType    条件操作符。具体值可参考当前对象的静态变量。
     * @param value           条件过滤值。
     * @return 验证结果对象，如果有错误将会返回具体的错误信息。
     */
    public Response setCriteria(String slaveTableName, String slaveColumnName, String fieldName, Integer operatorType, Object value) {
        this.operatorType = operatorType;
        this.fieldName = fieldName;
        this.value = value;
        return doVerify();
     ...
     * @return 验证结果对象，如果有错误将会返回具体的错误信息。
     */
    public Response setCriteria(Class<?> modelClazz, String fieldName, Integer operatorType, Object value) {
        this.modelClazz = modelClazz;
        this.operatorType = operatorType;
        this.fieldName = fieldName;
        this.value = value;
     ...
     * @return 验证结果对象，如果有错误将会返回具体的错误信息。
     */
    public Response doVerify() {
        if (fieldName == null) {
            return Response.error("过滤字段名称 [fieldName] 不能为空！");
        }
        ...
        if (!checkOperatorType()) {
            return Response.error("无效的操作符类型 [" + operatorType + "]!");
        }
        // 其他操作符必须包含value值
        if (operatorType != OPERATOR_IS_NULL && operatorType != OPERATOR_NOT_NULL && value == null) {
            String operatorString = this.getOperatorString();
            return Response.error("操作符 [" + operatorString + "] 的条件值不能为空！");
        }
        if (this.operatorType == OPERATOR_IN) {
            if (!(value instanceof Collection)) {
                return Response.error("操作符 [IN] 的条件值必须为集合对象！");
            }
            if (CollectionUtils.isEmpty((Collection<?>) value)) {
                return Response.error("操作符 [IN] 的条件值不能为空！");
            }
        }
