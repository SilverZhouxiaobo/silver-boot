...
import cn.silver.framework.online.dao.OnlineRuleMapper;
import cn.silver.framework.online.domain.OnlineColumnRule;
import cn.silver.framework.online.domain.OnlineRule;
import cn.silver.framework.online.domain.constant.GlobalDeletedFlag;
import cn.silver.framework.online.service.OnlineRuleService;
...
public class OnlineRuleServiceImpl extends BaseServiceImpl<OnlineRuleMapper, OnlineRule> implements OnlineRuleService {
...
//    }
    ...
    public OnlineRule saveNew(OnlineRule onlineRule) {
        onlineRule.setRuleId(IdUtil.randomUUID());
        Date now = new Date();
...
//        MyModelUtil.setDefaultValue(onlineRule, "pattern", "");
    ...
    public boolean update(OnlineRule onlineRule, OnlineRule originalOnlineRule) {
        ...
        return onlineRuleMapper.update(onlineRule) == 1;
    }
    ...
    public boolean remove(String ruleId) {
        OnlineRule onlineRule = new OnlineRule();
        onlineRule.setRuleId(ruleId);
        if (onlineRuleMapper.delete(onlineRule) == 0) {
            return false;
        }
        // 开始删除多对多父表的关联
        OnlineColumnRule onlineColumnRule = new OnlineColumnRule();
        onlineColumnRule.setRuleId(ruleId);
        onlineColumnRuleMapper.delete(onlineColumnRule);
        return true;
    }
