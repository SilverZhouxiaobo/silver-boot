...
import cn.silver.framework.online.config.CoreProperties;
import cn.silver.framework.online.config.DataFilterProperties;
import cn.silver.framework.online.config.OnlineProperties;
import cn.silver.framework.online.dao.OnlineOperationMapper;
import cn.hb.software.gacim.online.domain.*;
import cn.hb.software.gacim.online.domain.constant.*;
import cn.silver.framework.online.obj.ConstDictInfo;
import cn.silver.framework.online.obj.JoinTableInfo;
import cn.silver.framework.online.obj.MyWhereCriteria;
import cn.silver.framework.online.service.OnlineDictService;
import cn.silver.framework.online.service.OnlineOperationService;
import cn.silver.framework.online.service.OnlineVirtualColumnService;
import cn.silver.framework.online.util.OnlineConstant;
...
import cn.silver.framework.online.domain.constant.*;
...
public class OnlineOperationServiceImpl implements OnlineOperationService {
    ...
    private OnlineProperties onlineProperties = new OnlineProperties();
    @Autowired
    private OnlineOperationMapper onlineOperationMapper;
    private DataFilterProperties dataFilterProperties = new DataFilterProperties();
    ...
    public Object saveNewAndSlaveRelation(
        ...
        for (Map.Entry<OnlineDatasourceRelation, List<List<ColumnData>>> entry : slaveDataListMap.entrySet()) {
            String masterColumnId = entry.getKey().getMasterColumnId();
            ...
            String slaveColumnId = entry.getKey().getSlaveColumnId();
            // 迭代关联中的数据集合
    ...
    public boolean delete(OnlineTable table, List<OnlineDatasourceRelation> relationList, String dataId) {
        ...
        for (OnlineDatasourceRelation relation : relationList) {
            ...
            if (slaveTable.getLogicDeleteColumn() == null) {
                List<OnlineFilter> slaveFilterList =
    ...
    public void forceDelete(OnlineTable table, OnlineColumn column, String columnValue) {
        List<OnlineFilter> filterList = this.makeDefaultFilter(table, column, columnValue);
    ...
    public List<Map<String, Object>> getSlaveDataList(
            OnlineDatasourceRelation relation, List<OnlineFilter> filterList, String orderBy) {
        OnlineTable slaveTable = relation.getSlaveTable();
        // 如果主表中包含逻辑删除，需要在这里补充到过滤字段中。
        if (slaveTable.getLogicDeleteColumn() != null) {
            if (filterList == null) {
    ...
    @Override
    public List<Map<String, Object>> getDictDataList(OnlineDict dict, List<OnlineFilter> filterList) {
    ...
    private void buildDataListWithDict(
        ...
        Set<String> dictIdSet = new HashSet<>();
        // 先找主表字段对字典的依赖。
        Multimap<String, String> dictColumnMap = LinkedHashMultimap.create();
    ...
    private void doBuildDataListWithDict(
            List<Map<String, Object>> resultList, Set<String> dictIdSet, Multimap<String, String> dictColumnMap) {
        ...
        for (OnlineDict dict : dictList) {
            ...
            for (String columnName : columnNameList) {
                ...
                if (dict.getDictType().equals(DictType.CUSTOM)) {
                    ...
                    List<OnlineFilter> filterList = new LinkedList<>();
                    ...
                    OnlineFilter inlistFilter = new OnlineFilter();
    ...
    private void makeupColumnValue(ColumnData columnData) {
        ...
        if (columnData.getColumn().getPrimaryKey()) {
            if (columnData.getColumnValue() == null) {
                if ("String".equals(columnData.getColumn().getObjectFieldType())) {
                    columnData.setColumnValue(IdUtil.randomUUID());
                } else {
                    columnData.setColumnValue(IdUtil.randomUUID());
                }
            }
            ...
            switch (columnData.getColumn().getFieldKind()) {
                ...
                case FieldKind.UPDATE_USER_ID:
                    ...
                    columnData.setColumnValue(loginUser.getId());
                    break;
    ...
    private List<OnlineFilter> makeDefaultFilter(OnlineTable table, OnlineColumn column, String columnValue) {
        List<OnlineFilter> filterList = new LinkedList<>();
        OnlineFilter dataIdFilter = new OnlineFilter();
