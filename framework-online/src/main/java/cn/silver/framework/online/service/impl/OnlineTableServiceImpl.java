...
import cn.silver.framework.online.dao.OnlineTableMapper;
...
public class OnlineTableServiceImpl extends BaseServiceImpl<OnlineTableMapper, OnlineTable> implements OnlineTableService {
...
//    }
    ...
    public OnlineTable saveNewFromSqlTable(SqlTable sqlTable) {
        ...
        onlineTable.setTableId(IdUtil.randomUUID());
...
//        UpdateWrapper<OnlineTable> uw = this.createUpdateQueryForNullValue(onlineTable, onlineTable.getTableId());
        return onlineTableMapper.update(onlineTable) == 1;
    }
    ...
    public boolean remove(String tableId) {
        OnlineTable onlineTable = new OnlineTable();
        onlineTable.setTableId(tableId);
        if (onlineTableMapper.delete(onlineTable) == 0) {
            return false;
        }
...
//        tableIdSet.forEach(this::evictTableCache);
     ...
     */
    @Override
    public List<OnlineTable> getOnlineTableList(Set<String> tableIdSet) {
        return onlineTableMapper.selectByIds(tableIdSet);
    }
...
//        this.buildRelationForDataList(resultList, MyRelationParam.normal(), batchSize);
     ...
     */
    @Override
    public List<OnlineTable> getOnlineTableListByDatasourceId(String datasourceId) {
...
//        }
        OnlineTable table = onlineTableMapper.selectByPrimaryKey(tableId);
        if (table == null) {
            return null;
        }
