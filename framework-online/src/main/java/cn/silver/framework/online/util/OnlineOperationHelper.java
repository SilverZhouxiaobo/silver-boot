...
import cn.silver.framework.online.domain.constant.FieldKind;
...
public class OnlineOperationHelper {
     ...
     * @return 数据源详情数据。
     */
    public ResponseResult verifyAndGetDatasource(String datasourceId) {
        String errorMessage;
     ...
     * @return 数据源的关联详情数据。
     */
    public ResponseResult<OnlineDatasourceRelation> verifyAndGetRelation(String datasourceId, String relationId) {
        String errorMessage;
        OnlineDatasourceRelation relation = onlineDatasourceRelationService.selectById(relationId);
     ...
     */
    public ResponseResult<OnlineDatasourceRelation> verifyAndGetOneToManyRelation(String datasourceId, String relationId) {
        String errorMessage;
        OnlineDatasourceRelation relation = onlineDatasourceRelationService.selectById(relationId);
        if (relation == null || !relation.getDatasourceId().equals(datasourceId)) {
            return ResponseResult.error(ErrorCodeEnum.DATA_NOT_EXIST);
    ...
    public ResponseResult<List<OnlineDatasourceRelation>> verifyAndGetRelationList(
            String datasourceId, Integer relationType) {
        String errorMessage;
        Set<String> datasourceIdSet = CollUtil.newHashSet(datasourceId);
    ...
    public ResponseResult<Map<OnlineDatasourceRelation, List<List<ColumnData>>>> buildSlaveDataList(
            String datasourceId, JSONObject slaveData) {
