...
public class SysRoleServiceImpl extends BaseServiceImpl<SysRoleMapper, SysRole> implements ISysRoleService {
    ...
    public int insert(SysRole role) {
        if (UserConstants.NOT_UNIQUE.equals(this.checkRoleNameUnique(role))) {
            throw new CustomException(ResponseEnum.ROLE_ADD_ERROR_EXIST_NAME);
        } else if (UserConstants.NOT_UNIQUE.equals(this.checkRoleKeyUnique(role))) {
            throw new CustomException(ResponseEnum.ROLE_ADD_ERROR_EXIST_AUTHORITY);
        }
        // 新增角色信息
    ...
    public int update(SysRole role) {
        ...
        if (UserConstants.NOT_UNIQUE.equals(this.checkRoleNameUnique(role))) {
            throw new CustomException(ResponseEnum.ROLE_UPDATE_ERROR_EXIST_NAME);
        } else if (UserConstants.NOT_UNIQUE.equals(this.checkRoleKeyUnique(role))) {
            throw new CustomException(ResponseEnum.ROLE_UPDATE_ERROR_EXIST_AUTHORITY);
        }
        // 修改角色信息
        super.update(role);
        // 删除角色与菜单关联
        this.roleMenuMapper.deleteRoleMenuByRoleId(role.getId());
    ...
    @Override
    @Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
    public int deleteBatch(Collection<String> roleIds) {
