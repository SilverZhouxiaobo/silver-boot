...
import cn.silver.framework.common.utils.bean.BeanValidators;
...
import cn.hb.software.gacim.system.mapper.*;
import cn.hb.software.gacim.system.service.*;
import cn.silver.framework.system.domain.*;
import cn.silver.framework.system.mapper.*;
...
public class SysUserServiceImpl extends BaseServiceImpl<SysUserMapper, SysUser> implements ISysUserService {
    ...
    public SysUser selectById(String userId) {
        ...
        if (org.apache.commons.collections4.CollectionUtils.isNotEmpty(postIds)) {
            List<SysPost> posts = this.postService.selectByIds(postIds);
            user.setPosts(posts);
            user.setPostIds(postIds.toArray(new String[]{}));
        } else {
            user.setPosts(new ArrayList<>(0));
        }
        ...
        user.setRoles(rolesRet);
        user.setRoleIds(rolesRet.stream().map(SysRole::getId).collect(Collectors.toList()).toArray(new String[]{}));
        return user;
    }
