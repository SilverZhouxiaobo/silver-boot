package cn.silver.framework.generator.util;
...
import cn.silver.framework.generator.config.GenConfig;
...
public class GenUtils {
     ...
     * 初始化表信息
     */
    public static void initTable(GenTable genTable, LoginUser operUser) {
        genTable.setClassName(convertClassName(genTable.getTableName()));
        genTable.setPackageName(getPackageName(genTable.getTableName()));
        genTable.setModuleName(getModuleName(genTable.getTableName()));
        genTable.setBusinessName(getBusinessName(genTable.getTableName()));
        genTable.setFunctionName(replaceText(genTable.getTableComment()));
        genTable.setFunctionAuthor(GenConfig.getAuthor());
        genTable.setCreateBy(operUser.getId());
        genTable.setCreateName(operUser.getNickName());
    ...
    public static void initColumnField(GenTableColumn column, GenTable table) {
        ...
        column.setQueryType(SearchType.EQ.getCode());
        ...
        // 查询字段
        if (!arraysContains(GenConstants.COLUMNNAME_NOT_QUERY, columnName) && !column.isPk() && !dataType.equals(GenConstants.HTML_TEXTAREA)) {
            column.setIsQuery(GenConstants.REQUIRE);
        }
        // 查询字段类型
        if (StringUtils.endsWithIgnoreCase(columnName, "name") || StringUtils.endsWithIgnoreCase(columnName, "title")) {
            column.setQueryType(SearchType.LIKE.getCode());
        }
        // 状态字段设置单选框
    ...
    public static String getModuleName(String tableName) {
        ...
        return StringUtils.substring(tableName, 0, lastIndex);
    }
    ...
    public static String getPackageName(String tableName) {
        int lastIndex = tableName.lastIndexOf("_");
        return GenConfig.getPackageName() + "." + StringUtils.substring(tableName, 0, lastIndex);
    }
